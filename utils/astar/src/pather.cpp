        ...
        for (unsigned int a = 0; a < tileCount; ++a)
        {
            for (unsigned int b = 0; b < worldDirection; ++b)
            {
                ...
                if (getEntity || getNearEntities)
                {
                    for (int i = 0; i < entitiesSize; i++)
                    {
                        if (pass == entities[i])
                        {
                            if (WorldAt(nx, ny) >= 0)
                            {
                                // --- NEW: запрет диагонали, если оба ортогональных соседа заблокированы (только для классической сетки) ---
                                if (mapType == GRID_CLASSIC && abs(dx[b]) == 1 && abs(dy[b]) == 1)
                                {
                                    // ортогональные координаты
                                    int16_t ox1x = x + dx[b]; int16_t ox1y = y;
                                    int16_t ox2x = x;           int16_t ox2y = y + dy[b];

                                    int pass1 = Passable(ox1x, ox1y);
                                    int pass2 = Passable(ox2x, ox2y);

                                    // если оба ортогональных непроходимы — запрещаем эту диагональ
                                    if (pass1 < 0 && pass2 < 0)
                                    {
                                        continue;
                                    }
                ...
                if (pass == Costs[a].tile_id)
                {
                    if (WorldAt(nx, ny) >= 0)
                    {
                        // --- NEW: аналогичная проверка для обычных тайлов ---
                        if (mapType == GRID_CLASSIC && abs(dx[b]) == 1 && abs(dy[b]) == 1)
                        {
                            int16_t ox1x = x + dx[b]; int16_t ox1y = y;
                            int16_t ox2x = x;           int16_t ox2y = y + dy[b];
